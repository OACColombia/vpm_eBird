...
vertical dotted gray line (weeks 1-101; from the first week of January 2018 to the first week of December 2019). Local persistence probability estimation ($\hat{\phi}$) will correspond to the weeks to the right of the dotted gray line ($n_{eBird} = 258$, $n_{SM}=32$).
...
Very similar trends! We can see evidently the difference of the observation noise parameter ($\hat{\tau}^2$), way higher for eBird data. We can also explore the difference calculating the Root Mean Square Error between the two time series. Recall that there are four parameters in the EGSS:
...
## Results for \~10 years (500 weeks) - selected
...
# Aditional example for other populations of snail kite and monthly estimation of persistence
...
## Iterate the process for months for each cell
...
## Results of monthly estimate of persistance in three populations of snail kites
...
```{r Figure phi other populations, eval=TRUE}
