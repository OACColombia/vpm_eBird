...
2005](https://conbio.onlinelibrary.wiley.com/doi/full/10.1111/j.1523-1739.2005.00283.x)), fitting robust continuous mechanistic state-space population models under density-independent ([Humbert *et al.*, 2009](https://nsojournals.onlinelibrary.wiley.com/doi/pdfdirect/10.1111/j.1600-0706.2009.17839.x)) or density-dependent dynamics ([Dennis & Ponciano, 2014](https://esajournals.onlinelibrary.wiley.com/doi/abs/10.1890/13-1486.1)). We focused on the recent (since 2018) expanded population of Snail Kite ([*Rostrhamus sociabilis plumbeus*](https://birdsoftheworld.org/bow/species/snakit/cur/introduction)) in northcentral Florida, US, a species with long-term monitoring efforts following standardized protocols that provide benchmark population trends (see Section 2.1 in the main text).  

Specifically, we fit two continuous versions of the discrete-time equal sampling Gompertz State-Space model:
...
![Asking Snail Kite in Florida, US](data_raw/SnailKite_Florida_access_eBird.png){width=50%} 
...
the observations without count, add distance $0$ to
...
last with data of 2024 (June). Since 2018, Snail kites reached more than 1000
records.
...
![Histogram of the number of eBird checklists per year for Florida Snail kites. Red lines indicate our temporal sampling bias threshold (more than 1000 checklists per year), concentrating our sampling between January 2018 and May 2024.](supporting/FigSI-1_HistogramTemporalBias.png)
...
vertical dotted gray line (week 95; October 2019). Local persistence 
probability estimation ($\hat{\phi}$) will correspond to the weeks to the 
right of the dotted gray line ($n_{eBird} = 242$, $n_{SM}=48$).

\clearpage

# Population viability monitoring framework and local persistence estimation - $\hat{\phi}$
...
First, load the data saved and filter `Time.t` between `1:95`.
...
with respect to the standardized monitoring ($\hat{\tau}_{SM}^2=0.02677$ $<<$
$\hat{\tau}_{eBird}^2=0.25248$). The expected change in log-abundance per week describe lower change in eBird ($\hat{\theta}_{eBird}=0.02924$) and higher changes in standardized monitoring weekly counts ($\hat{\theta}_{SM}=0.04894$).
...
And make the figure 2. We combine the predicted trajectories in a single
...
![Weekly predicted (triangles) and observed (circles) high counts of the number of Snail Kites from standardized monitoring (orange) and eBird (blue). Predicted values correspond to Exponential Growth State-Space (EGSS) models fitted for the two datasets during population establishment (week 1: January 2018 to week 95: October 2019) in northcentral Florida (a). The predicted trajectory of standardized monitoring data matched closely the weekly high counts data ($y=-0.63+0.93x; R^2=0.985$), while the predicted trajectory from eBird data was far low of the identity ($y=2.3+0.075x; R^2=0.45$) compared with observed standardized monitored high counts; solid black line in (b) is the identity (1:1 relationship). *Figure 2 in Main text*](results/Figure2ab_EstimatedTrajectory2initdays.png){width=100%}

\FloatBarrier

## Three timeframe (~3, ~5, ~10) of first estimation $\hat{\phi}$
...
for (i in seq_along(len.sim)){
  ...
  for(n in 1:ntraj){
    ...
    phi[n] <- 1-(below.threshold/len)
        ...
        if( phi[n]>=0.5){
      lines(m+thres.times, Pop.sim, col="#d3d3d308", lty = "solid")
    }else{lines(m+thres.times, Pop.sim, col="#FF000008", lty = "solid")
    }
  }
  ...
  #above the threshold
  lines(x = (kde.sims$values*(ntraj/5) + m + last(thres.times)),
      y = kde.sims$grid_points,
      col="#d3d3d398", lwd=2, lty=1)

    lines(x = (kde.sims$values[kde.sims$grid_points <= N.critical]*(ntraj/5) + m + last(thres.times)),
...
parameter $\hat{\theta}$ to fit the EGSS model instead.
...
### Iterate the process with eBird (some figure examples)

Let's iterate the process for some (3) periods that share data in eBird
      ...
      # How many times each trajectory go below the threshold?
        ...
        if(phi[n] >= 0.5){
          lines(m+thres.times, Pop.sim, col="#d3d3d308", lty = "solid")
            ...
            lines(m+thres.times, Pop.sim, col="#FF000008", lty = "solid")
            }
    }
      # Add critical value line
      abline(h=N.critical, lty=2, lwd=1);
...
### Iterate each week to extract $\hat{\phi}$ and SD from eBird (no figures)
...
![Iterating all](data_raw/IterateAllNoFigs.png){width=50%}

\...

![Iterating all](data_raw/IterateAllNoFigs2.png){width=50%}
...
for (j in seq_along(end_positions)) {
  ...
  png(png_filename, width = 10, height = 5, units = "in", res = 300)
    ...
    if(model == "OUSS"){
      ...
      lines(x = (kde.sims$values[kde.sims$grid_points <= N.criticalSM]*(ntraj/5) + m + last(thres.times)),
...
How is changing in 3 moments $\hat{\phi}_{\sim 3\ years}$?
...
How is changing in 3 moments $\hat{\phi}_{\sim 5\ years}$?
...
![Estimation of local persistence probability ($\hat{\phi}$ +/- standard deviation) estimated for each dataset after 50,000 population trajectories simulated for three timeframes (a-c). Shape of the points shows the model selected in each week of estimation. Higher temporal resolution evident in eBird (n = 242) when compared with standardized monitored data (n = 48). See extended supplementary figure S6 with the observed abundance time series. *Figure 3 in Main text*](results/Figure3_PLocalPersi_3-5-10yrsSimulated.png){width=100%}
...
for (p in percentages) {
  ...
  end_positions <- which(tt_sampled >= 96) #having the initial phi fixed
...
![Time taken for different percentage](results/Time_taken_Sensitivity.png){width=50%}
...
sensilab <- c(
  'All' = "Complete (n = 242)",
  'p95' = "95% dataset (n = 228)",
  'p90' = "90% dataset (n = 213)",
  'p85' = "85% dataset (n = 205)",
  'p80' = "80% dataset (n = 196)",
  'p75' = "75% dataset (n = 178)",
  'p70' = "70% dataset (n = 172)",
  'p65' = "65% dataset (n = 157)",
  'p60' = "60% dataset (n = 147)",
  'p55' = "55% dataset (n = 134)",
  'p50' = "50% dataset (n = 124)",
  'p45' = "45% dataset (n = 113)",
  'p40' = "40% dataset (n = 97)",
  'p35' = "35% dataset (n = 86)",
  'p30' = "30% dataset (n = 70)",
  'p25' = "25% dataset (n = 59)",
...
![Sensitivity analysis. Correlation between eBird and standardized monitoring, from complete to reduced to 90, 70, 50, 30, and 10%](results/Figure4_Sensitivity_correlations.png){width=100%}
...
![Screenshot of the initial of the iteration](data_raw/OtherPopsEstimatingPhi.png){width=50%}
