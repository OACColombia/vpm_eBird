...
## 3.3. EGSS-REMLE (Some problems)

From ***H_O***. The numerical optimization for the REMLEs in `R` is:
...
## 3.7. Parametric bootstrap functions
...
Now we focused on the sampling unit with higher number of checklists, which correspond to the Payne's Prairie State Park wetland system in Alachua County. For this locality, we also have counts published from Poli *et.al.* (2020).

```{r eval=F}
#The monitoring data by Poly et al. (2020)
Poli.etal <- data.frame(observation.date = c("2018-02-19",
                                             ...
                                             "2018-10-15",
                                             "2018-12-16"),
                        Time.t = c(8, 10, 14, 19, 22, 23, 29, 32, 34, 35, 42, 50),
...
snailkites.paynesp <- snailkites.paynesp |>
  left_join(select(Poli.etal, c(Time.t,abundance.monitored)))
...
snailkites.paynesp.init <- snailkites.paynesp |>
  filter(Time.t %in% c(1:104))

```

Lets fit a first EGSS model
```{r}

snailkites.egss.parms <- egss_mle(yt = log(snailkites.paynesp.init$Observed.y),
                                  tt = snailkites.paynesp.init$Time.t,
                                  fguess_egss = guess_egss(log(snailkites.paynesp.init$Observed.y),
                                                           snailkites.paynesp.init$Time.t))

snailkites.egss.predict <- egss_predict(yt = log(snailkites.paynesp.init$Observed.y),
                                        tt = snailkites.paynesp.init$Time.t,
                                        parms = snailkites.egss.parms$mles)

#To return dates
datesPP <- snailkites.week |> 
  group_by(Time.t) |> 
  summarise(observation_date = min(observation_date))

snailkites.egss.predict <- snailkites.egss.predict |>
  left_join(snailkites.paynesp.init) |>
  left_join(datesPP)

plot(x = snailkites.egss.predict$observation_date,
     y = snailkites.egss.predict$Observed.y,
     type = "b", cex.lab=1.25, col = "blue", pch = 1,
     ylab = "Population abundance",
     xlab = "Observation date (weeks)")
points(x = snailkites.egss.predict$observation_date,
     y = snailkites.egss.predict$REMLE,
     type = "b", col = "darkred", pch = 2)
points(x = snailkites.egss.predict$observation_date,
     y = snailkites.egss.predict$abundance.monitored,
     type = "p", col = "black", pch = 19)
legend("topleft", legend=c("Observed eBird", "Predicted EGSS - REMLE", "Standardized Monitored"),
       col=c("blue","darkred", "black"), lty=c(1,1,NA), pch = c(1,2,19), cex=0.8)

#or
  
ggplot(snailkites.egss.predict, aes(x = observation_date, y = Observed.y))+
  geom_line(color = "blue", alpha = 0.5)+
  geom_point(color = "blue", shape = 21, alpha = 0.5)+
  geom_line(aes(y = REMLE), color = "red", alpha = 0.5)+
  geom_point(aes(y = REMLE), color = "red", shape = 21, alpha = 0.5)+
  geom_line(aes(y = abundance.monitored), color = "black", alpha = 0.5)+
  geom_point(aes(y = abundance.monitored), color = "black")+
...
factor_underestimation <- lm(REMLE~abundance.monitored, data = snailkites.egss.predict)

plot(x = snailkites.egss.predict$abundance.monitored,
     y = snailkites.egss.predict$REMLE,
     xlim=c(0,30), ylim=c(0,30), type = "p", col = "black", pch = 19,
     ylab = "Abundance observed (Standardized methods)",
     xlab = "Abundance estimated (REMLE - EGSS)")
abline(coef = c(0,1))
...
mtext(eq, 3, line=-16.5, col = "darkred")
...
```{r}
#variables:
tt <- snailkites.paynesp$Time.t
yt <- log(snailkites.paynesp$Observed.y)
last.tt <- length(snailkites.paynesp.init$Time.t)
...
points(x = snailkites.egss.predict$Time.t,
     y = snailkites.egss.predict$REMLE,
     type = "b", col = "darkred", pch = 2)
points(x = snailkites.egss.predict$Time.t,
     y = snailkites.egss.predict$abundance.monitored,
     type = "p", col = "black", pch = 19)
legend("topleft", legend=c("Observed eBird", 
                           "Predicted EGSS - REMLE", 
                           "Standardized Monitored",
                           "Projected above threshold",
                           "Projected below threshold"
                           ),
       col=c("blue","darkred", "black","#d3d3d398","#FF000098"
             ), 
       lty=c(1,2,NA,1,1
             ), pch = c(1,1,19,NA,NA
                        ), cex=0.8)
...
title(main=paste0("EGSS - P(ext risk) = ", signif(P.ext, digits=2)), cex=1.5)

```

Now, we can add the next observation (week 262; fist week of January 2020) to the time-series, and the model parameters are re-estimated as well as the probability of crashing below the $N_{critical}$ for the next two years (104 weeks). For this week, we will try to fit a OUSS as well, and decide by the density dependence parameter
...
EGSS.partial <- egss_mle(yt = yt[1:last.tt], 
                         ...
                         fguess_egss = guess_egss(yt = yt[1:last.tt],
                                                  tt = tt[1:last.tt]))
snailkites.egss.predict1 <- egss_predict(yt = yt[1:last.tt],
                                         tt = tt[1:last.tt],
                                         parms = EGSS.partial$mles)
...
plot(tt[1:last.tt], 
     ...
     ylim=c(0,90), 
     ylab="Population abundance", xlab="Time (week since 2018-01-01)")
points(x = snailkites.egss.predict1$Time.t,
     y = snailkites.egss.predict1$REMLE,
     type = "b", col = "darkred", pch = 2)
legend("topleft", legend=c("Observed eBird", 
                           "Predicted EGSS - REMLE", 
                           "Projected above threshold",
       ...
       col=c("blue","darkred","#d3d3d398","#FF000098"
             ), 
       lty=c(1,2,1,1
             ), pch = c(1,1,NA,NA
                        ), cex=0.8)
...
Iterating this process, for instance nine moments every 6 months (26 weeks):

```{r}

N.critical <- 1/2*mean(snailkites.paynesp$Observed.y);

ntraj = 1000

#A vector of time steps to evaluate
  last.tt <- seq(95,300, 25)
...
#array to store p.below critical point (last point < N.critical/ntraj)
  ...
  for(t in last.tt){
    ...
    if(model == "OUSS"){
      ...
      title(main=paste0(tt[t], " - ", model, " - P(ext risk) = ", signif(probability.ext1[t], digits=2)), cex=1.5)}
...
plot(probability.ext1, 
     xlim=c(min(tt),max(tt)),
     type = "b", ylim = c(0,1), 
     ylab = "Probability of extinction risk", 
...
```{r}
snailkites.pp.pvm <- vpm(yt, tt, ntraj = 1000, N.critical = N.critical, plt = 104)

saveRDS(snailkites.pp.pvm, "SnailKites_PaynesPrairie_PVM.rds")
```
