...
```{r eval=F}
saveRDS(SnailKiteCountsDay, "data_tmp/SnailKiteHighCountsDay.rds")
```

Let see if we can fit a REMLE OUSS for this population

```{r}
snailkites.ouss.remle <- ouss_calc(yt = log(SnailKiteCountsDay$Observed.y),
                           tt = SnailKiteCountsDay$Time.t,
                           pmethod = "REML",
                           nboot = 10,
                           plot.pred = F,
                           plot.bootdists = T)
