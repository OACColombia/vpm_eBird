...
This file is the code used for a paper *to be submitted* to ***Methods
in Ecology and Evolution***. Thus, we hope this program is applied for
different users in an intuitive and practical way. Our aim is to describe a framework to estimate risk-based population viability from community science data. Using a case study, we compare trends in estimated local persistence probability from community science data with those estimated from robust long-term monitoring (standardized monitoring). Then, we provide a sensitivity analysis to show how estimates accuracy varies as the number of observations in the community science datasets decreases. To estimate risk-based population viability ([Staples *et al.*,
2005](https://conbio.onlinelibrary.wiley.com/doi/full/10.1111/j.1523-1739.2005.00283.x)), we applied robust continuous mechanistic population models ([Dennis & Ponciano, 2014](https://esajournals.onlinelibrary.wiley.com/doi/abs/10.1890/13-1486.1); [Humbert *et al.*, 2009](https://nsojournals.onlinelibrary.wiley.com/doi/pdfdirect/10.1111/j.1600-0706.2009.17839.x)) to time series data from the community science platform [eBird](https://ebird.org/home). We focused on the recent (since 2018) expanded population of Snail Kite ([*Rostrhamus sociabilis plumbeus*](https://birdsoftheworld.org/bow/species/snakit/cur/introduction)) in northcentral Florida, US, a species with long-term monitoring efforts following standardized protocols that provide benchmark population trends (see Section 2.2 in the main text).  

Specifically, we fit continuous versions of the discrete-time equal sampling
Gompertz State-Space model for unequal sampling:
...
We applied a risk-based viable population monitoring framework (see
[Staples *et al.*,
2005](https://conbio.onlinelibrary.wiley.com/doi/full/10.1111/j.1523-1739.2005.00283.x)),
estimating the probability of local persistence $\mathit{\phi}$
(1-probability of crashing abundance below a specified threshold given a timeframe).

The statistical properties for our proceedings are based on [Dennis *et
al.*
(1991)](https://esajournals.onlinelibrary.wiley.com/doi/10.2307/1943004),
[Dennis *et al.*
(2006)](https://esajournals.onlinelibrary.wiley.com/doi/10.1890/0012-9615%282006%2976%5B323%3Aeddpna%5D2.0.co%3B2),
[Dennis & Ponciano,
(2014)](https://esajournals.onlinelibrary.wiley.com/doi/abs/10.1890/13-1486.1),
[Humbert *et al.*,
(2009)](https://nsojournals.onlinelibrary.wiley.com/doi/pdfdirect/10.1111/j.1600-0706.2009.17839.x),
and reference therein. We adjusted the functions and code published as
supplementary information in [Humbert *et al.*,
(2009)](https://nsojournals.onlinelibrary.wiley.com/doi/pdfdirect/10.1111/j.1600-0706.2009.17839.x)
and [Dennis & Ponciano,
(2014)](https://esajournals.onlinelibrary.wiley.com/doi/abs/10.1890/13-1486.1).
Thus, first of all run all the packages and functions in the first
section.
...
This model assumes density independence (not influence of the number of
individuals), and it could be of an stochastic form (see [Dennis *et al.* (2006)](https://esajournals.onlinelibrary.wiley.com/doi/10.1890/0012-9615%282006%2976%5B323%3Aeddpna%5D2.0.co%3B2)
and [Humbert *et al.*, (2009)](https://nsojournals.onlinelibrary.wiley.com/doi/pdfdirect/10.1111/j.1600-0706.2009.17839.x)).
The model is defined as the stochastic differential equation (from Itô
transformation):

$$
\begin{array}{ccc}
dX(t) &=& (\ln \lambda dt-\frac{1}{2} \beta^2)dt+\beta dW(t) \\
&=& \theta\ dt+\beta dW(t)
\end{array}
$$ 

where $X(t) = \ln{N(t)}$ or the log-normal population abundance at
time $t$, $\theta = \ln{\lambda}-\frac{1}{2}\beta^2$ is a constant of
...
although this notation is confusing for the OUSS notation), and $dW(t)$ is a random
perturbation representing the environmental stochasticity or process
noise (Itô transformation of $E_t$ from GSS in [Dennis *et al.* (2006)](https://esajournals.onlinelibrary.wiley.com/doi/10.1890/0012-9615%282006%2976%5B323%3Aeddpna%5D2.0.co%3B2)),
with mean $0$ and variance $\sigma^2dt$. Estimated or observed abundance
$Y(t_i)$ of the logarithmic population abundance $X(t_i)$ has an
observation error $F_i$ with mean $0$ and variance $\tau^2$:
...
The numerical optimization for the Restricted Maximum Likelihood
Estimation of parameters in `R` is:
...
With the EGSS-REMLE values, we can estimate the trajectory with the
function `egss_predict()`
...
The continuous version of GSS for unequal sampling is the OUSS model
(see [Dennis *et al.* (2006)](https://esajournals.onlinelibrary.wiley.com/doi/10.1890/0012-9615%282006%2976%5B323%3Aeddpna%5D2.0.co%3B2)).
Thus, let's define the stochastic version of GSS model as the stochastic
...
where $dW(t)$ is a random perturbation representing the environmental
stochasticity or process noise, with mean $0$ and variance of $dt$, with
intensity of noise scaled by the term $\beta N(t)$, with $\beta >0$.

Under the diffusion process, the infinitesimal mean function is given by
$m(n) = \theta n(\ln{\kappa}-\ln{n})$, and infinitesimal variance
function $v(n) = \beta^2 n^2$. As $X(t) = \ln{N_t}$, the infinitesimal
mean ($m_X$) and variance ($v_X$) are given by Itô transformation as
(***Eq4*** & ***Eq5*** in [Dennis & Ponciano, (2014)](https://esajournals.onlinelibrary.wiley.com/doi/abs/10.1890/13-1486.1)):
...
The OUSS model is defined as the stochastic differential equation
(***Eq6*** in [Dennis & Ponciano, (2014)](https://esajournals.onlinelibrary.wiley.com/doi/abs/10.1890/13-1486.1))

$$
dX(t) = \theta[\mu-X(t)]dt+\beta dW(t)
$$

Observed abundance $Y(t_i)$ of the logarithmic population abundance
$X(t_i)$ has an observation error $F_i$ with mean $0$ and variance
$\tau^2$:
...
The OUSS model have a joint multivariate normal distribution that in its
stationary form have four unknown parameters: $\mu$ (in stationary
distribution is the expected value of stationary distribution of
log-abundance mean; it is different than $\mu_{91}$ in [Dennis *et al.* (1991)](https://esajournals.onlinelibrary.wiley.com/doi/10.2307/1943004),
named here $\theta$ in *EGSS Functions*), $\theta$ (rate to approach to
stationarity), $\beta^2$ (variability of process noise), and $\tau^2$
(variability of sampling). The relationship of these parameters with
discrete GSS is given by the transformations:
...
We might assume that the population have become stationary at the moment
of first sampling, fluctuating around some equilibrium ($\mu$). Thus,
the initial log-abundance of the population $X(0)$ is part of the
stationary normal distribution (log-normal distribution of the
abundance, recall $X(0)=\ln(N(0))$), and its multivariate normal
log-likelihood for the stationary OUSS model is given by (see ***Eq. 19*** [Dennis & Ponciano, (2014)](https://esajournals.onlinelibrary.wiley.com/doi/abs/10.1890/13-1486.1)).
...
$V[Y(t_i)] = \tau^2+\frac{\beta^2}{2\theta}$; ***Eq. 17*** in [Dennis & Ponciano, (2014)](https://esajournals.onlinelibrary.wiley.com/doi/abs/10.1890/13-1486.1)),
...
The numerical optimization for the Restricted Maximum Likelihood
Estimation of parameters within the multivariate log-likelihood for the
...
With the OUSS-REMLE values, we can estimate the trajectory with the
...
Note that you have to download the `ebd` file from
[eBird](https://ebird.org/data/download). See the supplement to
[Johnston *et al.* (2021)](https://doi.org/10.1111/ddi.13271) in
[Strimas-Mackey *et al.* (2023)](https://ebird.github.io/ebird-best-practices/).
...
![Sign in eBird](data_raw/Sign%20in%20eBird.png)

If you are in the home page, check you are sign-in and move down on the
page to "Request data"

![eBird home](data_raw/eBird_home.png) ![Request
data](data_raw/eBird_home_down.png)
...
![Snail Kite in Florida, US](data_raw/SnailKite_Florida_access_eBird.png) In the options, include
the sampling event data always is a good recommendation. In Snail Kite
for US it will save the sampling event, but for other Neotropical
...
overwrite to assess different species.
...
The construction of time-series of the estimation of individuals from
...
hours, note the units are minutes), and only complete lists
...
![head of the ebd_filt file](data_raw/ebd_filt_head.png)

Then, just for the sake of double check and organization, we can remove
the observations without count estimation, add distance $0$ to
stationary protocols, modify the time of observations started to decimal
and round hour sampling to an integer, extract year, month, week, and
day_of_year. Also, we can confirm and filter out by effort, such as
observers $≤10$, distance $≤5 \text{ km}$, duration $≤5 \text{ hours}$,
and only records with abundance estimation.
...
![head of the ebd_filt file - note new columns were added](data_raw/ebd_filt_head2.png)
...
![head of the ebd_filt file - adding cell](data_raw/ebd_filt_head_cell.png)

This file is saved as a buckup
...
## 3.4. Time-series weekly high-counts

We adjust the time-series from the 1st week of 2018 (January) to the
22nd of 2024 (May). Since 2018, Snail kites reached more than 1000
records (Figure SI1).

```{r temporal bias}
SnailKite <- readRDS("data_tmp/SnailKiteCellsID_filtered.rds")

png('supporting/FigSI-1_HistogramTemporalBias.png',
...
![Figure SI-1. Histogram of the number of eBird checklists per year for Florida Snail kites. Red lines indicate our temporal sampling bias threshold (more than 1000 checklists per year), concentrating our sampling between January 2018 and May 2024.](supporting/FigSI-1_HistogramTemporalBias.png)
...
![Two areas surveyed by Snail Kite project](data_raw/PP%20and%20PPC%20map-min.png) 
...
with two variables and 440 observations, the count of observations per
each cell.
...
Some aesthetics are defined by the log-scales and arrows
...
Combine with the published data to have a single Standardized Monitored
...
Payne's Prairie (`CellTop$cell`). To avoid conflict with
`observation.date`, we generated an object of dates from the minimum
value of `snailkites.week$observation_date`, and join to the subset of
Snail kites high count per week in Payne's Praire
(`snailkites.paynesp`).
...
Finally, we add the standardized monitored (sorted by week id `Time.t`).
**CAUTION** if you do not sort by time, some functions will crash and not run
correctly, also make sure that you have only one value per time-step
...
snailkites.PP |>
  ...
  ggplot(aes(x = observation.date, y = Abundance, fill = group))+
    geom_vline(xintercept = snailkites.PP$observation.date[86], 
...
Assumed initial population dynamic is depicted to the left of the vertical 
dotted gray line (week 95; October 2019). Local persistence probability 
estimation ($\hat{\phi}$) will correspond to the weeks to the right of the 
dotted gray line (n = 242).

# 6. Population viability monitoring framework and local persistence estimation - ϕ̂

In the next section, we provide a graphical synthesis of the iterated process 
for each week estimation of local persistence probability ($\hat{\phi}$).

## 6.1. Fit dynamic population model to a first part of the time-series

Let's fit an EGSS model for the first two years – weeks from 1 to 95, n = 86, 
matching Standardized monitored data and published data ([Poli *et al.*, 2020](https://bioone.org/journals/the-wilson-journal-of-ornithology/volume-132/issue-1/1559-4491-132.1.183/Recent-breeding-range-expansion-of-the-endangered-Snail-Kite-Rostrhamus/10.1676/1559-4491-132.1.183.full)).

First, load the data saved and filter between `1:95`.
...
the object list (named here `sk.egss.parms`). The values correspond (in
order) to the trend parameter ($\hat{\theta}_{eBird}=0.02924$), the
environmental noise ($\hat{\sigma^2}_{eBird}=0.03161$), observation
error noise ($\hat{\tau^2}_{eBird}=0.25248$), and initial population
($\hat{x_0}_{eBird}=-0.01407$; note that $e^{x_0}\approx1$).

With the `egss_predict()` function, we can estimate the trajectory for
the EGSS model.
...
predicted (estimated trajectory) and observed trajectory. The `head()`
of the object shows the vector time (`Time.t`, week since January 2018,
in our case), the estimated abundance (`Predict.EGSS.REML`), and the
...
($\hat{\theta}_{SM}=0.04894$), the environmental noise
($\hat{\sigma}_{SM}^2=0.02954$), observation error noise
($\hat{\tau}_{SM}^2=0.02677$), and initial population
($\hat{x_0}=0.13109$). It is interesting that $\hat{\sigma}_{SM}^2$ is
similar ($\sim 0.03$) to $\hat{\sigma}_{eBird}^2$, as well as
$e^{x_0 = 0.131}_{SM}\approx1$ and $e^{x_0 = -0.014}_{eBird}\approx1$,
but the observation error is an order of magnitude bigger in eBird data
with respect to standardized monitored ($\hat{\tau}_{SM}^2=0.02677$ $<<$
$\hat{\tau}_{eBird}^2=0.25248$). The underestimation of population
abundance in eBird reflects the differences in trend parameter
($\hat{\theta}_{SM}=0.04894$ $>$ $\hat{\theta}_{eBird}=0.02924$)

Let's estimate the trajectory and change the column names for the
figure.
...
And make the figure 2. We combine the estimated trajectories in a single
...
FigS2a <- sk.Estimated |>
  ...
  ggplot(aes(x = observation.date, 
    ...
    geom_point(aes(shape = factor(group,
               ...
               alpha = 0.75, size = 2)+
    labs(x = "Observation date",
       y = "Observed/estimated population abundance",
       tag = expression(bold("(a)")),
       fill = "",
       color = "",
       shape = "",
    ...
    scale_color_manual(values = c("white", "#fc8d59","white", "#91bfdb"),
                       labels = c("Standardized Monitoring (SM)", "Estimated trajectory (SM - EGSS)", "eBird weekly high count", "Estimated trajectory (eBird - EGSS)"))+
...
How much is the prediction out of the estimation? Lets generate Figure
2b (we will use inside the code for the figure our function `lm_eqn()` to 
draw the equation).
...
FigS2b <- ggplot(sk.Estimated)+
  ...
  labs(x = "Observed population abundance (SM)",
...
![**Figure 2**. Snail Kite population abundance estimates (circles) from standardized monitoring (orange) and eBird (blue), with estimated trajectories (triangles and dashed lines) from Exponential Growth State-Space models in two years of breeding establishment in northcentral Florida (a); Poli et al. (2020) published data is depicted to the left of the vertical dotted gray line. The estimated trajectory of population abundance is slightly underestimated for standardized monitoring data (underestimation factor \~7%) and highly underestimated for eBird data (underestimation factor \~92.5%) compared with observed standardized monitored abundance; solid black line in (b) is the line of identity (relation 1:1).](results/Figure2ab_EstimatedTrajectory2initdays.png)

## 6.2. Three timeframe (~3, ~5, ~10) of first estimation ϕ̂

With the estimated model parameters and the data up to almost the 2
first years (up to week 95 with observations), we estimated the
probability that the population declines to values below a critical
threshold ($N_{critical}$, as the critical number of individuals for
assume pseudoextinction), in three different periods or given timeframes
(\~3 years $150$ weeks, \~5 years $250$ weeks, and \~10 years $500$ weeks). 
Let assume $N_{critical} = \frac{1}{2} (e^{\bar{y}}) = 5.58$. The probability 
of persistence ($\phi$) will be the mathematical complement of the number
of time steps within the three different periods (150, 250, 500) that
the population was lower than the threshold in 50,000 simulations. The
resulting probability $\phi$ is recorded for each trajectory, estimating the
...
#the first two years length (matching SM data), or last position, where is `m`
last.tt <- 86

#Define N_critical for eBird - 15% quantile
...
timeframes <- c("150 weeks or ~3 years", "250 weeks or ~5 years", "500 weeks or ~10 years")
```
...
for (i in seq_along(len.sim)){
  ...
  #above the threshold
  lines(x = (kde.sims$values*100 + m + last(thres.times)),
      y = kde.sims$grid_points,
      col="#d3d3d398", lwd=2, lty=1)

    lines(x = (kde.sims$values[kde.sims$grid_points <= N.critical]*100 + m + last(thres.times)),
...
We can see graphically the trajectories and the difference of $\phi$. The
figures show the observed weekly high counts (blue circles), the fitted
...
![Figure of the first round of trajectories - \~3 years](supporting/FigS2_Step2_1stTrajectories_10_5_150.png) 

![Figure of the first round of trajectories - \~5 years](supporting/FigS2_Step2_1stTrajectories_10_5_250.png) 

![Figure of the first round of trajectories - \~10 years](supporting/FigS2_Step2_1stTrajectories_10_5_500.png) 

## 6.3. Add the next observations, repeat model fit, projection, and ϕ̂

We can add the next week of observations (week 96; ending October 2019)
to the time-series, re-estimate model parameters as well as the
probability of crashing below the $N_{critical}$ during the four
timeframes (150, 250, 500 weeks). We tried to fit the OUSS model
for this new updated data set, and decided by the density dependence
parameter $\theta$ to fit the EGSS model instead.
...
log-abundance ($\hat{\mu}_{eBird}=1.28408$); the trend parameter, rate
to approach stationary distribution, or density dependence parameter
($\hat{\theta}_{eBird}=0.00012$); the environmental noise
($\hat{\sigma}^2_{SM}=0.03485$); and the observation error noise
($\hat{\tau}^2_{SM}=0.24859$). Note that the density dependence
parameter $\hat{\theta}<0.025$, which suggests density independence (we
should use EGSS).
...
#Lets fit EGSS model partial for second observation (week 96)
...
![Figure of the first round of trajectories - \~3 years](supporting/FigS3_Step3_2ndTrajectories_10_5_150.png) 

![Figure of the first round of trajectories - \~5 years](supporting/FigS3_Step3_2ndTrajectories_10_5_250.png) 

![Figure of the first round of trajectories - \~10 years](supporting/FigS3_Step3_2ndTrajectories_10_5_500.png) 

## 6.4. Iterate the process

### 6.4.1. Iterate the process with eBird (some figure examples)

Let's iterate the process for some (5) periods that share data in eBird
and Standardized Monitored. This is going to change the ending position;
instead of manually change 95 to 96, we use a vector of the "ending
...
end_positions <- c(86,139,200,252,305) 
...
for (j in seq_along(end_positions)) {
  ...
  png(png_filename, width = 10, height = 5, units = "in", res = 300)
    ...
    plot(tt[1:last.tt],
         ...
         xlab="Time (week since 2018-01-01)");
      ...
      # How many times each trajectory go below the threshold?
        ...
        if(phi[n] >= 0.5){
          lines(m+thres.times, Pop.sim, col="#d3d3d318", lty = "solid")
            ...
            lines(m+thres.times, Pop.sim, col="#FF000018", lty = "solid")
            }
    }
      # Add critical value line
      abline(h=N.critical, lty=2, lwd=1);
  ...
  print(cbind(m, model, phi_mean, phi_SD, len.sim[i]-1))
}
...
![iterating some weeks](data_raw/IteratingSome.png)

#### 4.1.1. Trajectories in timeframe of ~3 years

Let's see the examples.

How is changing in 5 moments $\hat{\phi}_{\sim 3\ years}$?

![Figure ~3 years - week 95 - October 2019](supporting/FigS4_Step4_IterateTraj_10_5_endpos_95_timeframe_150.png)

![Figure ~3 years - week 148 - October 2020](supporting/FigS4_Step4_IterateTraj_10_5_endpos_148_timeframe_150.png)

![Figure ~3 years - week 211 - January 2022](supporting/FigS4_Step4_IterateTraj_10_5_endpos_211_timeframe_150.png)

![Figure ~3 years - week 263 - January 2023](supporting/FigS4_Step4_IterateTraj_10_5_endpos_263_timeframe_150.png)

![Figure 1.5 years - week 316 - January 2024](supporting/FigS4_Step4_IterateTraj_10_5_endpos_316_timeframe_150.png)

#### 4.1.2. Trajectories in timeframe of ~5 years

How is changing in 5 moments $\hat{\phi}_{\sim 5\ years}$?

![Figure ~5 years - week 95 - October 2019](supporting/FigS4_Step4_IterateTraj_10_5_endpos_95_timeframe_250.png)

![Figure ~5 years - week 148 - October 2020](supporting/FigS4_Step4_IterateTraj_10_5_endpos_148_timeframe_250.png)

![Figure ~5 years - week 211 - January 2022](supporting/FigS4_Step4_IterateTraj_10_5_endpos_211_timeframe_250.png)

![Figure ~5 years - week 263 - January 2023](supporting/FigS4_Step4_IterateTraj_10_5_endpos_263_timeframe_250.png)

![Figure ~5 years - week 316 - January 2024](supporting/FigS4_Step4_IterateTraj_10_5_endpos_316_timeframe_250.png)

#### 4.1.3. Trajectories in timeframe of ~10 years

How is changing in 5 moments $\hat{\phi}_{\sim 10\ years}$?

![Figure ~10 years - week 95 - October 2019](supporting/FigS4_Step4_IterateTraj_10_5_endpos_95_timeframe_500.png)

![Figure ~10 years - week 148 - October 2020](supporting/FigS4_Step4_IterateTraj_10_5_endpos_148_timeframe_500.png)

![Figure ~10 years - week 211 - January 2022](supporting/FigS4_Step4_IterateTraj_10_5_endpos_211_timeframe_500.png)

![Figure ~10 years - week 263 - January 2023](supporting/FigS4_Step4_IterateTraj_10_5_endpos_263_timeframe_500.png)

![Figure ~10 years - week 316 - January 2024](supporting/FigS4_Step4_IterateTraj_10_5_endpos_316_timeframe_500.png)

### 6.4.2. Iterate each week to extract φ and SD from eBird (no figures)

```{r iterate process eBird NO figures but data, eval=FALSE}
#Change at the end to `{r eval=FALSE}`; it migh take long time
...
end_positions <- which(tt >= 95)

#check weeks selected as `end_positions`
tt[end_positions]
...
![Iterating all](data_raw/IterateAllNoFigs.png)

\...

![Iterating all](data_raw/IterateAllNoFigs2.png)

This is a longer process (\~9 hours; currently is `{r eval=FALSE}`), so
...
![head screenshot of eBird phi estimations](data_raw/eBird_phi_hat_SD_head.png)

### 6.4.3. Iterate the process with Standardized Monitored (figure examples)
...
#N crticial
N.criticalSM <- (1/2)*mean(exp(ytSM))
...
for (j in seq_along(end_positions)) {
  ...
  png(png_filename, width = 10, height = 5, units = "in", res = 300)
    ...
    if(model == "OUSS"){
      ...
      lines(x = (kde.sims$values[kde.sims$grid_points <= N.criticalSM]*100 + m + last(thres.times)),
...
![iteration for the standardized monitored](data_raw/IterateAllSM.png)
...
#### 6.4.3.1. Trajectories in timeframe of ~3 years

Let's see the examples.

How is changing in 5 moments $\hat{\phi}_{\sim 3\ years}$?

![Figure ~3 years - week 95 - October 2019](supporting/FigS5_IterateTrajSM_10_5_endpos_95_timeframe_150.png)

![Figure ~3 years - week 148 - October 2020](supporting/FigS5_IterateTrajSM_10_5_endpos_148_timeframe_150.png)

![Figure ~3 years - week 211 - January 2022](supporting/FigS5_IterateTrajSM_10_5_endpos_211_timeframe_150.png)

![Figure ~3 years - week 263 - January 2023](supporting/FigS5_IterateTrajSM_10_5_endpos_263_timeframe_150.png)

![Figure ~3 years - week 316 - January 2024](supporting/FigS5_IterateTrajSM_10_5_endpos_316_timeframe_150.png)

#### 6.4.3.2. Trajectories in timeframe of ~5 years

How is changing in 5 moments $\hat{\phi}_{\sim 5\ years}$?

![Figure ~5 years - week 95 - October 2019](supporting/FigS5_IterateTrajSM_10_5_endpos_95_timeframe_250.png)

![Figure ~5 years - week 148 - October 2020](supporting/FigS5_IterateTrajSM_10_5_endpos_148_timeframe_250.png)

![Figure ~5 years - week 211 - January 2022](supporting/FigS5_IterateTrajSM_10_5_endpos_211_timeframe_250.png)

![Figure ~5 years - week 263 - January 2023](supporting/FigS5_IterateTrajSM_10_5_endpos_263_timeframe_250.png)

![Figure ~5 years - week 316 - January 2024](supporting/FigS5_IterateTrajSM_10_5_endpos_316_timeframe_250.png)

#### 6.4.3.3. Trajectories in timeframe of ~10 years

How is changing in 5 moments $\hat{\phi}_{\sim 3\ years}$?

![Figure ~10 years - week 95 - October 2019](supporting/FigS5_IterateTrajSM_10_5_endpos_95_timeframe_500.png)

![Figure ~10 years - week 148 - October 2020](supporting/FigS5_IterateTrajSM_10_5_endpos_148_timeframe_500.png)

![Figure ~10 years - week 211 - January 2022](supporting/FigS5_IterateTrajSM_10_5_endpos_211_timeframe_500.png)

![Figure ~10 years - week 263 - January 2023](supporting/FigS5_IterateTrajSM_10_5_endpos_263_timeframe_500.png)

![Figure ~10 years - week 316 - January 2024](supporting/FigS5_IterateTrajSM_10_5_endpos_316_timeframe_500.png)
...
#Load data saved
  ...
  N.critical <- 5.58104 # 1/2 empirical K = 1/2 mean(exp(yt))
  N.criticalSM <- 32.05102 # 1/2 empirical K = 1/2 mean(exp(ytSM))
...
phi_data_plot <- datesPP |> 
  left_join(phi_combined|> filter(Timeframe == "150 weeks or ~3 years")) |>
  ungroup() |>
  pivot_longer(cols = !c(Timeframe, Time.t, observation.date, Model),
               names_to = "group",
...
Figure3a <- ggplot(phi_data_plot, 
  ...
  scale_x_date(limits = c(snailkites.PP$observation.date[86],
...
ggsave("results/Figure3_PLocalPersi_3-5-10yrsSimulated.pdf", 
       plot = Figure3, dpi = 300, width = 10, height = 9, units = "in")

ggsave("results/Figure3_PLocalPersi_3-5-10yrsSimulated.png", 
       plot = Figure3, dpi = 300, width = 10, height = 9, units = "in")
```

![**Figure 3.** Estimation of local persistence probability (ϕ̂̂ +/- standard deviation) estimated for each dataset after 50,000 population trajectories simulated for three timeframes (a-c). Shape of the points shows the model selected in each week of estimation. Higher temporal resolution evident in eBird (n = 242) when compared with standardized monitored data (n = 48). See extended supplementary figure S6 with the observed abundance time series.](results/Figure3_PLocalPersi_3-5-10yrsSimulated.png)

How correlated are the $\hat{\phi}$?

```{r correlation in three timeframes}
correlations <- phi_combined %>%
  group_by(Timeframe) %>%
...
### 6.5.1. Results for \~3 years (150 weeks)
...
Fig3aExt <- ggplot(phi_data_plot |> 
  ...
  geom_point(data = phi_data_plot |> 
             ...
             color = "black", size = 1, fill = "#4575b4", shape = 21)+
  geom_point(data = phi_data_plot |> 
                  filter(group %in% c("phi.hat")) |>
               drop_na(Model),
             aes(shape = Model), color = "black") +
  labs(x = "Observation date",
       y = "Population abundance",
       tag = expression(bold("(a)")),
       fill = "",
...
Fig3bExt <- ggplot(phi_data_plot |> 
  ...
  geom_point(data = phi_data_plot |> 
             ...
             color = "black", size = 1, fill = "#d73027", shape = 21)+
  geom_point(data = phi_data_plot |> 
...
![Time series of weekly high counts of Snail kites in Payne's Prairie wetland from eBird (a) and standardized monitoring surveys (b), with local persistence probability (φ) +/- standard deviation estimated for each dataset after 50,000 population trajectories simulated. (c) Local persistence probability extimated from eBird data plotted against local persistence probability estimated from standardized monitored, with standard deviation. Estimation of φ from 50,000 trajectories in a timeframe of \~3 years (150 weeks). Solid black line is the line of identity, while dashed black line is linear regression (equation in gray).](results/Figure3_timeseries_PLocalPersi150weeks.png)

### 6.5.2. Results for \~5 years (250 weeks) - selected
...
![**Figure 3**. Time series of weekly high counts of Snail kites in Payne's Prairie wetland from eBird (a) and standardized monitoring surveys (b), with local persistence probability (φ) and 95% confidence intervals estimated for each dataset after 10,000 bootstraps of 50,000 population trajectories simulated (upper and lower dashed lines reflect original confidence intervals without bootstrap resampling). (c) Local persistence probability extimated from eBird data plotted against local persistence probability estimated from standardized monitored, with bootstrapped confidence intervals. Estimation of φ from 50,000 trajectories in a timeframe of \~5 years (250 weeks). Solid black line is the line of identity, while dashed gray line is linear regression (equation in gray).](results/Figure3_timeseries_PLocalPersi250weeks.png)

### 6.5.3. Results for \~10 years (500 weeks)
...
![**Figure 3**. Time series of weekly high counts of Snail kites in Payne's Prairie wetland from eBird (a) and standardized monitoring surveys (b), with local persistence probability (φ) and 95% confidence intervals estimated for each dataset after 10,000 bootstraps of 50,000 population trajectories simulated (upper and lower dashed lines reflect original confidence intervals without bootstrap resampling). (c) Local persistence probability extimated from eBird data plotted against local persistence probability estimated from standardized monitored, with bootstrapped confidence intervals. Estimation of φ from 50,000 trajectories in a timeframe of \~10 years (500 weeks). Solid black line is the line of identity, while dashed gray line is linear regression (equation in gray).](results/Figure3_timeseries_PLocalPersi500weeks.png)

## 6.6. Sensitivity analysis

As a sensitivity analysis, we randomly removed 90, 70, 50, 30, and 10%
of the data in eBird, and projected under the timeframe of \~5 years
(250 weeks). This process last ~5 hours.
...
percentages <- c(0.9, 0.7, 0.5, 0.3, 0.1)
...
for (p in percentages) {
  ...
  end_positions <- which(tt_sampled >= 95)
...
![end of the `results` list](data_raw/sensitivity_results_list.png)
...
![head and tail of the sensitivity data frame](results/SensitivityDF.png)
...
sensilab <- c(
  'All' = "Complete eBird dataset (n = 242)",
  'p90' = "90% of eBird dataset (n = 218)",
  'p70' = "70% of eBird dataset (n = 171)",
  'p50' = "50% of eBird dataset (n = 117)",
  'p30' = "30% of eBird dataset (n = 79)",
  'p10' = "10% of eBird dataset (n = 20)"
)
...
Figure4 <- ggplot(cor_sen,
  ...
  stat_regline_equation(label.x = 0.25, label.y = 0.2) +
  stat_cor(aes(label=..rr.label..), label.x = 0.35, label.y = 0.1)+
...
ggsave("results/Figure4_Sensitivity_correlations.pdf", 
       plot = Figure4, dpi = 300, width = 7, height = 6, units = "in")

ggsave("results/Figure4_Sensitivity_correlations.png", 
       plot = Figure4, dpi = 300, width = 7, height = 6, units = "in")
```

![Sensitivity analysis. Correlation between eBird and standardized monitoring, from complete to reduced to 90, 70, 50, 30, and 10%](results/Figure4_Sensitivity_correlations.png)
...
#lets saved it with good proportions
ggsave("supporting/Figure4ext_sensitivity_datareduction.pdf", 
       plot = Fig4Ext, dpi = 300, width = 10, height = 7, units = "in")

ggsave("supporting/Figure4ext_sensitivity_datareduction.png", 
       plot = Fig4Ext, dpi = 300, width = 10, height = 7, units = "in")

```

![**Figure 4 - Extended**. Data reduction as sensitivity analysis. Estimation of local persistence probability (ϕ) for standardized monitoring (orange) is replicated in each panel. Sequence of panels represent reduction from the complete dataset (same as Figure 3b) to different percentages of randomly subsampled data, from top-left to bottom-right. Point shapes represent fitted model, the EGSS (circle) or OUSS (triangle).](supporting/Figure4ext_sensitivity_datareduction.png)


# 7. Other populations of Snail Kite

```{r filtering other populations}
SnailKite |>
  group_by(cell) |>
  filter(year >= 2018) |>
  summarise(count = n(), 
...
![Screenshot of the initial of the iteration](data_raw/OtherPopsEstimatingPhi.png)
...
phiSD_df_pops |> 
  ...
  scale_shape_manual(values = c(21,24))+
  labs(x = "Observation date",
       y = "ϕ - estimated with monthly high-counts in eBird")+
...
# 8. Searching eBird data availability

We conducted a search to the eBird data for species currently availability of weekly high count. This search require the use of a super computer facility (384 GB of 16 CPUs required), but species-specific search will provide a better understanding of the potential mechanisms driving population trends by assessing risk-based dynamics.

```{r searching_eBird_counts_SuperCompu, eval=FALSE}
#In a super computer 
...
f_ebd <- "ebd_Examples.txt"

initial_filter <- auk_ebd("ebd_relMay-2024.txt") |>
  auk_year(year = years) |>
  auk_protocol(c("Traveling", "Stationary")) |>
  auk_distance(distance = c(0,5)) |>
  auk_duration(duration = c(0,300)) |>
...
saveRDS(initial_filter, "filter_eBird_2018_2024_standard.rds")

all_spp_availability_eBird <- initial_filter |>
  mutate(
    # We don't want here count in 'X', to convert to NA we use `as.integer()`
    observation_count = as.integer(observation_count),
    # effort_distance_km to 0 for non-travelling counts
  ...
  mutate(cell = dgGEO_to_SEQNUM(dggs_pop,
                                longitude, latitude)$seqnum) %>%
  group_by(scientific_name, cell, year, month) %>%
  mutate(max_count = max(observation_count, na.rm = T),
         n_lists = n()) |>
  ungroup() |>
  mutate(Time.t = case_when(year == 2018 ~ month,
...
saveRDS(all_spp_availability_eBird, "count_week_2018_eBirds_world.rds")
#End of this code
